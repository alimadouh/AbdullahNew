...
<div class="page">
  ...
  <p style="margin-top:32px;font-size:11px;color:#999;text-align:center">Generated by Medical Guidance — ${dateStr}</p>
</div>
<script>
          ...
          <div className="flex flex-col gap-3">
                            ...
                            return (
                              ...
                              >
                                ...
                                {item.ref && isFailed && totalMonths >= item.ref && (
                                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-red-100 text-red-600 font-semibold shrink-0 dark:bg-red-900/40 dark:text-red-300">REFER</span>
                                )}
                              </button>
                            )
                          })}
            ...
            {redFlags.length > 0 && (
              <div className="border rounded-xl overflow-hidden" style={{ borderColor: '#dc262630' }}>
                ...
                {!collapsed.redFlags && (
                  <div>
                    <div className="px-4 py-1 text-[10px] font-bold uppercase tracking-wider text-red-400 bg-red-50/50 dark:bg-red-950/40">
                      Check if present
                    </div>
                    {redFlags.map((rf, i) => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-4 py-2 text-left cursor-pointer hover:bg-red-50/50 dark:hover:bg-red-950/40 transition-colors border-b last:border-b-0"
                        ...
                        >
                          ...
                          <span className={`text-sm ${isChecked ? 'text-red-700 dark:text-red-300 font-medium' : ''}`}>{rf.text}</span>
                        </button>
                      )
                    })}
                  </div>
            ...
            {assessment && (
              <div className="border rounded-xl overflow-hidden">
                <div className="px-4 py-2.5 bg-gray-50 dark:bg-white/5 font-semibold text-sm flex items-center gap-2">
                  Assessment
                ...
                <div className="p-4 flex flex-col gap-3">
                  {assessment.onTrack ? (
                    <div className="flex items-center gap-2 text-green-600 bg-green-50 rounded-lg p-3 dark:text-green-400 dark:bg-green-950/30">
                      <CheckCircle2 className="h-5 w-5 shrink-0" />
                      <div>
                        <span className="font-semibold">Development on track</span>
                        <p className="text-xs text-green-700 dark:text-green-300 mt-0.5">All expected milestones achieved. No red flags.</p>
                      </div>
                    </div>
                  ) : (
                    <>
                      {assessment.delays.length > 0 && (
                        <div className="bg-amber-50 rounded-lg p-3 border border-amber-200 dark:bg-amber-950/30 dark:border-amber-900/50">
                          <p className="text-sm font-semibold text-amber-700 dark:text-amber-300 mb-1">⚠️ Delayed Milestones</p>
                          {assessment.delays.map((d, i) => (
                            <div key={i} className="mb-1">
                              <span className="text-xs font-bold" style={{ color: d.color }}>{d.category}: </span>
                              <span className="text-xs text-amber-800 dark:text-amber-200">{d.items.map(item => item.text).join(', ')}</span>
                            </div>
                          ))}
                        </div>
                      )}
                      {assessment.referrals.length > 0 && (
                        <div className="bg-red-50 rounded-lg p-3 border border-red-200 dark:bg-red-950/30 dark:border-red-900/50">
                          <p className="text-sm font-semibold text-red-700 dark:text-red-300 mb-1 flex items-center gap-1">
                            ...
                            <div key={i} className="mb-2">
                              ...
                              {r.items.map((item, j) => (
                                <div key={j} className="ml-2 mt-0.5">
                                  <p className="text-xs text-red-800 dark:text-red-300 font-medium">• {item.text}</p>
                      ...
                      {assessment.redFlagConcerns.length > 0 && (
                        <div className="bg-red-50 rounded-lg p-3 border border-red-200 dark:bg-red-950/30 dark:border-red-900/50">
                          ...
                          {assessment.redFlagConcerns.map((rf, i) => (
                            <p key={i} className="text-xs text-red-800 dark:text-red-300 ml-2">• {rf.text}</p>
                          ))}
                        </div>
                      )}
                    </>
